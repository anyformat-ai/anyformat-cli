...
func init() {
	Command = &cli.Command{
		Name:      "anyformat",
		Usage:     "CLI for the anyformat API",
		Suggest:   true,
		Version:   Version,
		...
		Flags: []cli.Flag{
			...
			&requestflag.Flag[string]{
				Name:    "api-key",
				Sources: cli.EnvVars("ANYFORMAT_API_KEY"),
			},
		},
		Commands: []*cli.Command{
			...
			},
			{
				...
				UsageText:       "anyformat @manpages [-o anyformat.1] [--gzip]",
				Hidden:          true,
...
func generateManpages(ctx context.Context, c *cli.Command) error {
	...
	if c.Bool("text") {
		file, err := os.Create(filepath.Join(dir, "man1", "anyformat.1"))
		if err != nil {
			return err
		}
		defer file.Close()
	...
	if c.Bool("gzip") {
		file, err := os.Create(filepath.Join(dir, "man1", "anyformat.1.gz"))
		if err != nil {
			return err
		}
		defer file.Close()
